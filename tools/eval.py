...
def eval_with_plac(img_dir, det_net, num_imgs, image_ext, draw_imgs=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        for i, a_img_name in enumerate(imgs):
            ...
            detected_indices_h = det_scores_h_ >= cfgs.VIS_SCORE
            detected_scores_h = det_scores_h_[detected_indices_h]
            detected_boxes_h = det_boxes_h_[detected_indices_h]
            detected_categories_h = det_category_h_[detected_indices_h]

            detected_indices_r = det_scores_r_ >= cfgs.VIS_SCORE
            detected_scores_r = det_scores_r_[detected_indices_r]
            detected_boxes_r = det_boxes_r_[detected_indices_r]
            detected_categories_r = det_category_r_[detected_indices_r]

            end = time.time()
            # print("{} cost time : {} ".format(img_name, (end - start)))
            if draw_imgs:
                det_detections_h = draw_box_in_img.draw_box_cv(np.squeeze(resized_img, 0),
                                                               boxes=detected_boxes_h,
                                                               labels=detected_categories_h,
                                                               scores=detected_scores_h)
                det_detections_r = draw_box_in_img.draw_rotate_box_cv(np.squeeze(resized_img, 0),
                                                                      boxes=detected_boxes_r,
                                                                      labels=detected_categories_r,
                                                                      scores=detected_scores_r)
                # det_detections_h = draw_box_in_img.draw_boxes_with_label_and_scores(np.squeeze(resized_img, 0),
                #                                                                     boxes=det_boxes_h_,
                ...
                cv2.imwrite(save_dir + '/' + a_img_name + '_h.png',
                            det_detections_h[:, :, ::-1])
                cv2.imwrite(save_dir + '/' + a_img_name + '_r.png',
                            det_detections_r[:, :, ::-1])

            # xmin, ymin, xmax, ymax = det_boxes_h_[:, 0], det_boxes_h_[:, 1], \
            #                          det_boxes_h_[:, 2], det_boxes_h_[:, 3]

            xmin, ymin, xmax, ymax = detected_boxes_h[:, 0], detected_boxes_h[:, 1], \
                                     detected_boxes_h[:, 2], detected_boxes_h[:, 3]

            if detected_boxes_r.shape[0] != 0:
                # resized_h, resized_w = resized_img.shape[1], resized_img.shape[2]
                resized_h = resized_img.shape[1]
                resized_w = resized_img.shape[2]
                detected_boxes_r = forward_convert(detected_boxes_r, False)
                detected_boxes_r[:, 0::2] *= (raw_w / resized_w)
                detected_boxes_r[:, 1::2] *= (raw_h / resized_h)
                detected_boxes_r = back_forward_convert(detected_boxes_r, False)

            x_c, y_c, w, h, theta = detected_boxes_r[:, 0], detected_boxes_r[:, 1], detected_boxes_r[:, 2], \
                                    detected_boxes_r[:, 3], detected_boxes_r[:, 4]
            ...
            dets_h = np.hstack((detected_categories_h.reshape(-1, 1),
                                detected_scores_h.reshape(-1, 1),
                                boxes_h))
            dets_r = np.hstack((detected_categories_r.reshape(-1, 1),
                                detected_scores_r.reshape(-1, 1),
                                boxes_r))
